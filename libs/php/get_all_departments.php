...
if (mysqli_connect_errno()) {
  ...
  $query = '
    SELECT department.id, department.name, location.name AS locationName
    FROM department
    LEFT JOIN location
  ...
  if ($result === false) {
    ...
    $output['status']['returnedIn'] = intval((microtime(true) - $executionStartTime) * 1000) . " ms";
    $output['data'] = null;
    error_log("Company Directory Get All Departments Query Failure",0);
    mysqli_close($conn);
  }
  else {
    // Success
...
if ($output['status']['name'] == "critical failure") {
  error_log("Company Directory Get All Departments Critical Failure");
}
